...
OUT_DOWN = Path("down_facilities.json")   # fetch_kosei が読む下り施設データ（アプリ用）
...
def write_down_json(rows):
    """アプリ用の下り施設データ（down_facilities.json）を出力。
    fetch_kosei.py がこれを読み、手動オーバーレイをマージして facilities.json に連結する。
    基本は全院採用の想定（uro_level で専門/対応/処方・近隣を区別）。"""
    out = []
    for i, r in enumerate(rows):
        caps = [c for c in r["ナビイ抽出cap"].split() if c]
        out.append({
            "id": f"dn{101 + i:03d}",      # 手動分(dn001〜)と衝突しない採番
            "name": r["施設名"], "dir": "down", "real": True,
            "lat": r["lat"], "lng": r["lng"],
            "addr": r["住所"], "tel": r["電話"], "url": r["HP URL"],
            "station": "", "walk": None, "bus": "", "bf": True,
            "hours": "紹介予約制（要確認）",
            "caps": caps,
            "uro_level": r["泌尿器専門度"],   # 専門 / 対応 / 処方・近隣
            "score": r["確度"],
            "source": "医療情報ネット(ナビイ)",
        })
    OUT_DOWN.write_text(json.dumps(out, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"  → {OUT_DOWN}（{len(out)}件・fetch_kosei がマージして facilities.json へ）")
...
def main():
    ...
    write_csv(rows)
    write_html(rows)
    write_down_json(rows)
    print("完了。navii_candidates.csv/html を医師がレビュー。down_facilities.json は fetch_kosei が取り込みます。")
